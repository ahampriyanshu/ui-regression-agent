...
async def run_regression_test(baseline_path: str, updated_path: str) -> Dict:
    ...
    try:
        logger.logger.info("Phase 1: Screenshot comparison and difference detection")
        differences = await ui_agent.compare_screenshots(baseline_path, updated_path)
        ...
        logger.logger.info(f"Found {len(differences['differences'])} differences")
        
        logger.logger.info("Phase 2: Classification and analysis against JIRA tickets")
        analysis = await classification_agent.analyze_differences(differences["differences"])
        
        logger.log_regression_analysis(baseline_path, updated_path, 
        ...
        logger.logger.error(f"UI regression test failed: {e}")
        return {
            "status": "error",
            "message": str(e),
...
def streamlit_interface():
    ...
    if st.button("🚀 Start UI Regression Analysis", type="primary"):
        ...
        with st.spinner("🔍 Analyzing screenshots and checking JIRA tickets..."):
            try:
                result = asyncio.run(run_regression_test(baseline_path, updated_path))
...
def main():
    ...
    if len(sys.argv) > 1:
        if len(sys.argv) != 3:
            print("Usage: python app.py <baseline_image_path> <updated_image_path>")
            print("Example: python app.py screenshots/login_baseline.png screenshots/login_updated.png")
